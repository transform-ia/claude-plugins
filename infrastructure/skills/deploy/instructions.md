...
```bash
/infrastructure:deploy                              # Dry-run all hosts
/infrastructure:deploy --apply                      # Apply changes
/infrastructure:deploy --limit command-center       # Dry-run one host
/infrastructure:deploy --apply --limit command-center  # Apply on one host
/infrastructure:deploy --tags docker                # Dry-run specific playbook
```
...
```bash
/infrastructure:host-status                    # Check all hosts
/infrastructure:host-status --host command-center  # Check one host
```
