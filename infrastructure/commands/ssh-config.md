---
description: "Sync SSH config from inventory: /infrastructure:ssh-config [--dry-run]"
allowed-tools: [Bash(${CLAUDE_PLUGIN_ROOT}/scripts/ssh-config.sh *)]
---
...
```text
Bash("${CLAUDE_PLUGIN_ROOT}/scripts/ssh-config.sh $ARGUMENTS")
```
