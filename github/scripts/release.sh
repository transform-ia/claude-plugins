...
# Usage: release.sh [directory] <version|patch|minor|major|auto>
set -euo pipefail
...
if [[ -z "$FIRST_ARG" ]]; then
    echo "Usage: /github:release [directory] <version|patch|minor|major|auto>" >&2
    exit 1
fi
...
# Call push-and-wait.sh to monitor workflows
# Use same repo detection
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
"${SCRIPT_DIR}/push-and-wait.sh" "$REPO"
