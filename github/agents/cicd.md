...
| Command                             | Purpose                         |
| ----------------------------------- | ------------------------------- |
| `/github:actionlint [dir]`          | Run yamllint + prettier         |
| `/github:workflow-status [repo]`    | Check workflow status           |
| `/github:logs <run-id>`             | Get workflow logs               |
| `/github:release <version>`         | Full release with build monitor |
...
instead (Go → go:gocode, Dockerfile → docker:container, Helm →
helm:agent-dev). Then stop - make no tool calls.

**Follow all instructions in `skills/cicd/instructions.md`**
