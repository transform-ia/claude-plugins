...
for script in enforce-helm-files.sh block-bash.sh cmd-lint.sh cmd-format.sh cmd-template.sh stop-lint-check.sh; do
...
echo '{"tool_name":"Write","tool_input":{"file_path":"/tmp/test.go"}}' | \
    TEST_CALLER="" "$SCRIPTS_DIR/enforce-helm-files.sh" && \
    pass "Allows writes when not in plugin context" || \
    fail "Hook scoping" "Should allow when not in helm context"

# Test: Hook scoping - should block .go files in plugin context
result=$(echo '{"tool_name":"Write","tool_input":{"file_path":"/tmp/test.go"}}' | \
    TEST_CALLER="/helm:skill-dev" "$SCRIPTS_DIR/enforce-helm-files.sh" 2>&1 || true)
...
echo '{"tool_name":"Write","tool_input":{"file_path":"/tmp/Chart.yaml"}}' | \
    TEST_CALLER="/helm:skill-dev" "$SCRIPTS_DIR/enforce-helm-files.sh" && \
...
echo '{"tool_input":{"command":"rm /tmp/Chart.yaml"}}' | \
    TEST_CALLER="/helm:skill-dev" "$SCRIPTS_DIR/block-bash.sh" && \
...
result=$(echo '{"tool_input":{"command":"rm /tmp/test.go"}}' | \
    TEST_CALLER="/helm:skill-dev" "$SCRIPTS_DIR/block-bash.sh" 2>&1 || true)
if [[ "$result" == *"BLOCKED"* ]]; then
