...
allowed-tools: [Bash(${CLAUDE_PLUGIN_ROOT}/scripts/image-tag.sh *), mcp__dockerhub__*]
---
...
Usage: /docker:image-tag `<image>` [count]" and STOP. Do not proceed with any
tool calls.
...
```text
Bash("${CLAUDE_PLUGIN_ROOT}/scripts/image-tag.sh $ARGUMENTS")
```
...
- `/docker:image-tag quay.io/superq/chrony-exporter` - Quay.io image
